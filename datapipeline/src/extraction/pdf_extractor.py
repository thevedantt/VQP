...
# Add src to sys.path to allow absolute/relative imports cleanly when executed from anywhere
...
class PDFExtractor:
    """
    Extracts text from PDF files using PyMuPDF (fitz) with a fallback to PyPDF.
    """
    ...
    def _check_libraries(self):
        ...
        try:
            ...
            logger.info("PyMuPDF (fitz) is available and will be used as the primary extractor.")
        except ImportError:
        ...
        try:
            import pypdf
            self.has_pypdf = True
            logger.info("PyPDF is available for fallback.")
        except ImportError:
            self.has_pypdf = False
            logger.warning("PyPDF is not installed.")

    def extract_text_pymupdf(self, pdf_path: Path) -> tuple[str, list[str], int]:
        ...
        doc.close()
        return "\n".join(text_list), text_list, page_count

    def extract_text_pypdf(self, pdf_path: Path) -> tuple[str, list[str], int]:
    ...
    def extract_pdf(self, pdf_path: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing extraction metadata and success flag.
        """
        ...
        if self.has_pymupdf:
            try:
                extracted_text, pages, page_count = self.extract_text_pymupdf(pdf_path)
                result["success"] = True
        ...
        if not result["success"] and self.fallback_to_pypdf and self.has_pypdf:
            try:
                ...
                extracted_text, pages, page_count = self.extract_text_pypdf(pdf_path)
                result["success"] = True
            except Exception as e:
